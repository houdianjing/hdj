...
/* 搜索接口说明
 *   搜索链接demo：/interface/search_api.php?action='search'&type=''&keywords=''&uid=54
...
if($action=='search') {
    ...
    //搜索词不为空 执行
        ...
        if ($searchType == 'store') {
            if (!empty($searchText)) {
                /* 获得商品总数 */
                $sql = "SELECT COUNT(*) FROM " . $ecs->table('supplier') . "  WHERE status = 1 and supplier_name LIKE '%$searchText%'";
                $count = $GLOBALS['db']->getOne($sql);
                if($count==0){
                    $result = array('code'=>104,'msg' => '店铺搜索结果为空');
                    exit(json_encode($result));
                }
                $sql = "SELECT supplier_id, supplier_name,rank_id " .
                    "FROM " . $ecs->table('supplier') .
                    "WHERE status = 1 and supplier_name LIKE '%$searchText%'";
                $res = $GLOBALS['db']->query($sql);
                $arr = array();
                //$arr['count'] = $count;
                while ($row = $db->FetchRow($res)) {
                    $arr['list'][$row['supplier_id']]['supplier_id'] = $row['supplier_id'];  //商店id
                    $arr['list'][$row['supplier_id']]['supplier_name'] = $row['supplier_name'];  //商店名称
                    $arr['list'][$row['supplier_id']]['rank_id'] = $row['rank_id'];  //商店等级
                    $arr['list'][$row['supplier_id']]['url'] = $weburl . "/" . "supplier.php?suppId=" . $row['supplier_id']; //商店链接
                }
